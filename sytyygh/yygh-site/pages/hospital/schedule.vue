    ...
    <div class="nav left-nav">
      ...
      <div class="nav-item">
        <span
          ...
          :onclick="'javascript:window.location=\'/hosp/detail/'+hoscode+'\''"
        >医院详情</span>
      </div>
      <div class="nav-item">
        <span
          class="v-link clickable dark"
          :onclick="'javascript:window.location=\'/hosp/notice/'+hoscode+'\''"
        >预约须知</span>
      </div>
      <div class="nav-item">
      ...
      <div class="hospital-source-list">
        ...
        <div class="countdown-wrapper mt60" v-if="!tabShow">
          <div class="countdonw-title">
            {{ time }}
            <span class="v-link selected">{{ baseMap.releaseTime }}</span>放号
          </div>
...
export default {
  ...
  methods: {
    ...
    getBookingScheduleRule() {
      hospitalApi
        ...
        .then(response => {
          ...
          this.dealClass(); // 分页后workDate=null，默认选中第一个
    ...
    selectDate(item, index) {
      ...
      this.activeIndex = index; //清理定时

      if (this.timer != null) clearInterval(this.timer); // 是否即将放号

      if (item.status == 1) {
        this.tabShow = false; // 放号时间
    ...
    convertTimeString(allseconds) {
      if (allseconds <= 0) return "00:00:00"; // 计算天数
      let days = Math.floor(allseconds / (60 * 60 * 24)); // 小时
      let hours = Math.floor((allseconds - days * 60 * 60 * 24) / (60 * 60)); // 分钟
      let minutes = Math.floor(
        (allseconds - days * 60 * 60 * 24 - hours * 60 * 60) / 60
      ); // 秒
      let seconds =
        allseconds - days * 60 * 60 * 24 - hours * 60 * 60 - minutes * 60; //拼接时间
